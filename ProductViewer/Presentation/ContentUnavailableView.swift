...
extension View {
    ...
    @ViewBuilder
    func contentUnavailable(when condition: Bool,  configuration: ContentUnavailableConfiguration,
        action: (() -> Void)? = nil) -> some View {
        if condition {
            ContentUnavailableWrapper(configuration: configuration, action: action)
        } else {
            self
        }
    }
